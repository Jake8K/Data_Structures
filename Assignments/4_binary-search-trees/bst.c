...
struct Node *_addNode(struct Node *cur, TYPE val) {
    ...
    return cur; 		  //return current (moded) node to caller to rebuild the tree
}
...
struct Node *_removeLeftMost(struct Node *cur) {
    ...
    if (cur->left == NULL) { 			//curent node == leftmost
            ...
            return temp;                    //return the right child to the caller
    }
...
/*****************************************************************************************
 fucntion to test that the BST contains the elements that we added to it // 4 allocs
